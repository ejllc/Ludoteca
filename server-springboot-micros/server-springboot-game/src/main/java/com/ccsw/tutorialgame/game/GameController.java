...
public class GameController {
	...
	@GetMapping("")
	public List<GameDto> find(@RequestParam(required = false) String title,
		...
		return gameService.find(title, idCategory).stream().map(game -> {
			...
			gameDto.setCategory(categories.stream().filter(category -> category.getId().equals(game.getIdCategory()))
	...
	@PutMapping({ "", "/{id}" })
	public void save(@PathVariable(required = false) Long id, @RequestBody GameDto dto) {
